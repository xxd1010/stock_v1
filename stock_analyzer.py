...
class StockAnalyzer:
    ...
    def get_stock_data_from_db(self, stock_code: str, start_date: str, end_date: str):
        ...
        Returns:
            pd.DataFrame: 股票数据
        """
        logger.info(f"从数据库获取股票数据: {stock_code}, 时间范围: {start_date} 至 {end_date}")
        
        # 从数据库获取数据
        stock_data = self.data_fetcher.get_stock_data_from_db(stock_code, start_date, end_date)
        
        if stock_data.empty:
            logger.error(f"未从数据库获取到股票 {stock_code} 的数据")
            return None
        
        logger.info(f"从数据库获取股票数据完成: {stock_code}")
        return stock_data
    ...
    def analyze_stock(self, stock_code: str, start_date: str, end_date: str):
        ...
        # 1. 从数据库获取数据
        stock_data = self.get_stock_data_from_db(stock_code, start_date, end_date)
...
def main():
    ...
    elif args.operation == "batch-analyze":
        # 批量分析股票
        if not all([args.start_date, args.end_date]):
            logger.error("batch-analyze 操作需要在配置文件中提供 start_date, end_date 参数")
            return
